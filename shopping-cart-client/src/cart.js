...
export default function () {
  ...
  const generateProducts = async () => {
    ...
    if (product_list) {
      ...
      products = await getData(
        `${globals.API_URL}/api/products?category_id=${category_id || ""}`
      );
  ...
  const totalAmount = () => {
    ...
    const cart_total_item = document.getElementById("cart_total_item");

    if (basket.length !== 0) {
      cart_total_item.innerHTML = basket
        .map((x) => x.item)
        .reduce((x, y) => x + y, 0);
      let amount = totalCartAmount(basket);
      cart_total_amount.innerHTML = amount;
    } else {
      if (cart_total_amount !== null) {
        cart_total_amount.innerHTML = 0;
      }
      cart_total_item.innerHTML = 0;
    }
  };
  ...
  const decrement = (id) => {
    const search = basket && basket.find((x) => x.id === id);
